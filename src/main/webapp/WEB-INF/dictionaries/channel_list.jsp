	...
	<blockquote class="layui-elem-quote news_search" style="margin-top: 10px">
		<div class="layui-inline">
		     <div class="layui-input-inline">
		    	<input id="channelName" type="text" value=""  placeholder="请输入渠道名称" class="layui-input search_input">
		    </div> 
		    <a onclick="queryChannel()" class="layui-btn search_btn" ><b>查询</b></a>
		
				<a class="layui-btn linksAdd_btn"  onclick="openWindow();" style="background-color:#5FB878"><b>新增渠道</b></a>
		</div>	
	</blockquote>
	<div class="table-responsive">
		<table class="table table-striped table-hover" id="channelTable" style="text-align:center"></table>
	</div> 
</body>
...
function initChannelTable(){ 
	$("#channelTable").bootstrapTable({
		url:getRootPath()+'/channel/get/list.do',//加载表格数据的接口
		method:'GET',//提交方式
...
function openWindow(){
	parent.layer.open({
		type:2,
		area:["50%","60%"],
		content:getRootPath()+"/channel/show/channel.do",
		btn:["确定","取消"],
...
function e(id){
	...
	parent.layer.open({
		type:2,
		area:["50%","60%"],
		content:getRootPath()+"/channel/show/channel.do?id="+id,
		btn:["确定","取消"],
		btnAlign:'c',
		yes:function(index,layero){
			var body = layer.getChildFrame('body', index);
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/channel/opt/channel.do",
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/channel/del/channel.do",
				success:function(data){
					layer.close(index);
					parent.layer.msg(data.message);
					location.reload(true);
