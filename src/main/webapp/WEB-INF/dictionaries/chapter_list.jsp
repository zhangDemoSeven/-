...
<body class="container-fluid">
		...
		<div class="layui-inline">
		    <div class="layui-input-inline">
		    	<input id="name" name="name" type="text" value="" placeholder="请输入章节名称" class="layui-input search_input">
		    </div>
		    <a class="layui-btn search_btn" onclick="searchChapter()"><b>查询</b></a>
		</div>
		<div class="layui-inline">
			<a class="layui-btn linksAdd_btn"  onclick="openWindow();" style="background-color:#5FB878"><b>新增章节</b></a>
		</div>
	</blockquote>
...
function initChapterTable(){ 
	$("#chapterTable").bootstrapTable({
		url:getRootPath()+'/chapter/get/list.do',//加载表格数据的接口
		method:'GET',//提交方式
		...
		columns:[
			...
			{field:'booksId',visible:false},//设置隐藏列
			{field:'name',title:'章节名称',align:'center'},
			{field:'stageName',title:'所属阶段',align:'center'},
			{field:'booksName',title:'所属课程',align:'center'},
...
function openWindow(){
	parent.layer.open({
		...
		type:2,
		area:["800px","500px"],
		content:getRootPath()+"/chapter/show/chapter.do",
		btn:["确定","取消"],
		btnAlign:'c',
		maxmin:true,
...
function e(id){
	...
	var rows = $("#chapterTable").bootstrapTable('getSelections');
	...
	parent.layer.open({
		type:2,
		area:["800px","500px"],
		content:getRootPath()+"/chapter/show/chapter.do?id="+id,
		btn:["确定","取消"],
		btnAlign:'c',
		maxmin:true,
		yes:function(index,layero){
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/chapter/opt/chapter.do",
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/chapter/del/chapter.do",
				success:function(data){
					layer.close(index);
					parent.layer.msg(data.message);
					location.reload(true);
