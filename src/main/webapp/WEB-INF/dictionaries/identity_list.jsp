...
<blockquote class="layui-elem-quote news_search" style="margin-top: 10px">
	<div class="layui-inline">
	     <div class="layui-input-inline">
	    	<input id="name" type="text" value=""  placeholder="请输入身份名称" class="layui-input search_input">
	    </div> 
	    <a onclick="queryIdentity()" class="layui-btn search_btn" ><b>查询</b></a>
	
		<div class="layui-inline" style="margin-top: 5px">
			<a class="layui-btn linksAdd_btn"  onclick="openWindow();" style="background-color:#5FB878"><b>新增身份</b></a>
		</div>
	</div>	
</blockquote>
<div class="table-responsive">
	<table class="table table-striped table-hover" id="identityTable" style="text-align:center"></table>
</div> 
...
function queryIdentity(){
	$("#identityTable").bootstrapTable('refresh');
}

function initIdentityTable(){ 
	$("#identityTable").bootstrapTable({
		url:getRootPath()+'/identity/get/list.do',//加载表格数据的接口
		method:'GET',//提交方式
		...
		columns:[
			...
			{field:'name',title:'身份名称',align:'center'},
...
function openWindow(){
	parent.layer.open({
		...
		type:2,
		area:["550px","200px"],
		content:getRootPath()+"/identity/show/identity.do",
		btn:["确定","取消"],
...
function e(id){
	...
	var rows = $("#identityTable").bootstrapTable('getSelections');
	...
	parent.layer.open({
		type:2,
		area:["550px","200px"],
		content:getRootPath()+"/identity/show/identity.do?id="+id,
		btn:["确定","取消"],
		btnAlign:'c',
		yes:function(index,layero){
			var body = layer.getChildFrame('body', index);
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/identity/opt/identity.do",
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/identity/del/identity.do",
				success:function(data){
					layer.close(index);
					parent.layer.msg(data.message);
					location.reload(true);
