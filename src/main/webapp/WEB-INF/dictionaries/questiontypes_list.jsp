	...
	<blockquote class="layui-elem-quote news_search" style="margin-top: 10px">
		<div class="layui-inline">
		     <div class="layui-input-inline">
		    	<input id="name" type="text" value=""  placeholder="请输入题型名称" class="layui-input search_input">
		    </div> 
		    <a onclick="queryQuestionTypes()" class="layui-btn search_btn" ><b>查询</b></a>
		
			<div class="layui-inline" style="margin-top: 5px">
				<a class="layui-btn linksAdd_btn"  onclick="openWindow();" style="background-color:#5FB878"><b>新增题型</b></a>
			</div>
		</div>	
	</blockquote>
	<div class="table-responsive">
		<table class="table table-striped table-hover" id="questiontypesTable" style="text-align:center"></table>
	</div> 
</body>
<script type="text/javascript">
$(function(){
	initQuestiontypesTable();
...
function queryQuestionTypes(){
	$("#questiontypesTable").bootstrapTable('refresh');
}

function initQuestiontypesTable(){ 
	$("#questiontypesTable").bootstrapTable({
		url:getRootPath()+'/questiontypes/get/list.do',//加载表格数据的接口
		method:'GET',//提交方式
		dataType:'json',//服务器返回的数据格式
		questiontypesbar:'#tb', //指定按钮组
		...
		columns:[
			...
			{field:'name',title:'题型名称',align:'center'},
			...
			{field:'modifyBy',title:'修改人',align:'center'},
...
function openWindow(){
	parent.layer.open({
		...
		type:2,
		area:["550px","200px"],
		content:getRootPath()+"/questiontypes/show/questiontypes.do",
		btn:["确定","取消"],
...
function e(id){
	...
	var rows = $("#questiontypesTable").bootstrapTable('getSelections');
	...
	parent.layer.open({
		type:2,
		area:["550px","200px"],
		content:getRootPath()+"/questiontypes/show/questiontypes.do?id="+id,
		btn:["确定","取消"],
		btnAlign:'c',
		yes:function(index,layero){
			var body = layer.getChildFrame('body', index);
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/questiontypes/opt/questiontypes.do",
	...
	parent.layer.confirm(
		...
		function(index){
			$.ajax({
				...
				type:'POST',
				data:{"id":id},
				url:getRootPath()+"/questiontypes/del/questiontypes.do",
				success:function(data){
					layer.close(index);
					parent.layer.msg(data.message);
					location.reload(true);
