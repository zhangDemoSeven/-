...
public class BaseService {
   ...
   public static boolean checkObjFieldIsNull(Object obj) throws CustomException, IllegalArgumentException, IllegalAccessException { 
	   for(Field f : obj.getClass().getDeclaredFields()){ 
		   f.setAccessible(true); //这里忽略static final 类型的属性，如若不需要可以去掉 
   ...
   /**
	...
	public static String editKey(String oldObject) {
		if(oldObject != null) {
			if(oldObject.contains("<")) {
				oldObject = oldObject.replaceAll("<", "&lt;");
			}
			if(oldObject.contains(">")) {
				oldObject = oldObject.replaceAll(">", "&gt;");
			}
	...
	/*public static String buildTitleToShow(String oldTitle) {
		if(oldTitle.contains("<span style='font-size:20px;font-weight:700'>")) {
			String replaceAll = oldTitle.replaceAll("<span style='font-size:20px;font-weight:700'>", "");
			if(replaceAll.contains("</span>")) {
				String newTitle = replaceAll.replaceAll("</span>", "");
